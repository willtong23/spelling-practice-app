...
document.addEventListener('DOMContentLoaded', function() {
    console.log("=== TYLER DASHBOARD INITIALIZING ===");
...
async function loadAllData() {
    console.log("=== LOADING ALL DATA FOR TYLER ===");
    ...
    try {
        ...
        console.log("Loading tyler's data...");
        await loadTylerData();
        ...
        console.log("=== ALL TYLER DATA LOADED SUCCESSFULLY ===");
...
// Load Tyler's data
async function loadTylerData() {
    try {
        console.log("=== LOADING TYLER'S DATA ===");
        
        // Load Tyler's student record
        const studentSnapshot = await window.db.collection("students").where("name", "==", "tyler").get();
        if (studentSnapshot.empty) {
            console.log("No student named 'tyler' found in students collection");
            students = [];
        } else {
            ...
            console.log("Tyler's student record loaded:", students[0]);
        }
        
        // Load Tyler's quiz results
        const resultsSnapshot = await window.db.collection("results").where("user", "==", "tyler").get();
        if (resultsSnapshot.empty) {
            console.log("No quiz results found for 'tyler'");
            results = [];
        } else {
            ...
            console.log(`Loaded ${results.length} quiz results for Tyler`);
...
function initializeAnalyticsFilters() {
    ...
    // Populate student filter with Tyler
    if (studentFilter) {
