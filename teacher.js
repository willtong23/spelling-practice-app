    ...
    container.innerHTML = `
        ...
        <div class="students-list">
            ${students.map(student => {
                ...
                // Check if student's default matches their class default
                const classDefaultWordSetId = studentClass?.defaultWordSetId;
                const hasClassDefault = classDefaultWordSetId && student.defaultWordSetId === classDefaultWordSetId;
                const classDefaultWordSet = classDefaultWordSetId ? wordSets.find(ws => ws.id === classDefaultWordSetId) : null;
                    ...
                    <div class="student-card">
                        ...
                        <div class="default-word-set-section">
                            <div class="default-set-header">
                                <span class="default-set-label">Default Word Set:</span>
                                <span class="default-set-name">${defaultWordSet ? defaultWordSet.name : 'None set'}</span>
                                ${hasClassDefault ? '<span class="class-default-indicator" title="This matches the class default">📚 Class Default</span>' : ''}
                                ${!student.defaultWordSetId && classDefaultWordSet ? `<span class="missing-default-warning" title="Class has default but student doesn\'t">⚠️ Missing (Class default: ${classDefaultWordSet.name})</span>` : ''}
                            </div>
                            <div class="default-set-controls">
                                <select class="default-set-select" id="studentDefaultSet_${student.id}">
                                    ...
                                    ${wordSets.map(ws => `
                                        <option value="${ws.id}" ${ws.id === student.defaultWordSetId ? 'selected' : ''}>
                                            ${ws.name} (${ws.words.length} words)${ws.id === classDefaultWordSetId ? ' - Class Default' : ''}
                                        </option>
                                    `).join('')}
                                </select>
                                ...
                                ${classDefaultWordSet && !hasClassDefault ? `
                                    <button class="btn-small btn-secondary" onclick="setStudentToClassDefault('${student.id}', '${classDefaultWordSetId}')" title="Set to class default: ${classDefaultWordSet.name}">
                                        Use Class Default
                                    </button>
                                ` : ''}
                            </div>
...
// Function to set student to use their class default
async function setStudentToClassDefault(studentId, classDefaultWordSetId) {
    try {
        const student = students.find(s => s.id === studentId);
        const wordSet = wordSets.find(ws => ws.id === classDefaultWordSetId);
        
        if (!student || !wordSet) {
            showNotification('❌ Student or word set not found', 'error');
            return;
        }
        
        // Update the student's default word set
        await window.db.collection('students').doc(studentId).update({
            defaultWordSetId: classDefaultWordSetId
        });
        
        // Update local data
        const studentIndex = students.findIndex(s => s.id === studentId);
        if (studentIndex !== -1) {
            students[studentIndex].defaultWordSetId = classDefaultWordSetId;
        }
        
        showNotification(`✅ ${student.name} now uses class default: ${wordSet.name}`, 'success');
        
        // Refresh the display
        renderStudentsAndClasses();
        
    } catch (error) {
        console.error('Error setting student to class default:', error);
        showNotification('❌ Error updating student default', 'error');
    }
}
...
async function setClassDefaultWordSet(classId) {
    try {
        ...
        // Always update all students in the class to have the same default
        if (studentsInClass.length > 0) {
            console.log(`Updating default word set for ${studentsInClass.length} students in class "${className}"`);
            ...
            for (const student of studentsInClass) {
                try {
                    ...
                    console.log(`Updated student ${student.name} with default word set: ${wordSetName}`);
                } catch (error) {
        ...
        if (updatedStudentsCount > 0) {
            showNotification(`✅ Class "${className}" default word set updated to "${wordSetName}". All ${updatedStudentsCount} students in this class now have this as their default word set.`, 'success');
        } else {
            showNotification(`✅ Class "${className}" default word set updated to "${wordSetName}".`, 'success');
        }
        
        // Force a complete re-render to show the updated defaults
        setTimeout(() => {
            renderStudentsAndClasses();
        }, 500);
...
function showCreateWordSetModal() {
    const content = `
        <div class="form-group">
            <label class="form-label">Set Name:</label>
            <input type="text" class="form-input" id="wordSetName" placeholder="e.g., Basic Words">
        </div>
        <div class="form-group">
            <label class="form-label">Description:</label>
