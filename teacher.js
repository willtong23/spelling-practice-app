...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (!connectionOk) {
    ...
    showNotification(
      "Failed to connect to Firebase database. Please check your internet connection.",
      "error",
    );
    return;
  }
    ...
    for (const collectionName of collections) {
      try {
        const snapshot = await window.db
          ...
          .get();
        console.log(
          `Collection '${collectionName}' accessible. Size:`,
          snapshot.size,
        );
      } catch (error) {
...
function renderWordSets() {
  ...
  if (wordSets.length === 0 && wordSetFolders.length === 0) {
    content =
      '<p style="text-align: center; color: #64748b; grid-column: 1/-1;">No word sets or folders created yet. Click "Create New Set" to get started.</p>';
  }
        ...
        <div class="classes-grid" id="classesGrid">
            ${classes
              .map((cls) => {
                ...
                const defaultWordSet = wordSets.find(
                  (ws) => ws.id === cls.defaultWordSetId,
                );
        ...
        <div class="students-grid maximum-separation-grid" id="studentsGrid" 
            ...
            ${students
              .map((student, index) => {
                ...
                const classAssignments = studentClass
                  ? assignments.filter((a) => a.classId === studentClass.id)
                  : [];
                const defaultWordSet = wordSets.find(
                  (ws) => ws.id === student.defaultWordSetId,
                );
                const totalAssignments =
                  studentAssignments.length + classAssignments.length;
...
function populateAssignmentSelects() {
  ...
  if (studentSelect) {
    studentSelect.innerHTML =
      ...
      students
        .map(
          (s) => `<option value="${s.id}">${s.displayName || s.name}</option>`,
        )
        .join(""); // Use displayName
  }
  ...
  const wordSetOptions = wordSets
    .map((ws) => `<option value="${ws.id}">${ws.name}</option>`)
    .join("");
  const assignWordSetSelect = document.getElementById("assignWordSetSelect");
  const assignClassWordSetSelect = document.getElementById(
    "assignClassWordSetSelect",
  );

  if (assignWordSetSelect) {
    assignWordSetSelect.innerHTML =
      '<option value="">Choose a word set...</option>' + wordSetOptions;
  }
  if (assignClassWordSetSelect) {
    assignClassWordSetSelect.innerHTML =
      '<option value="">Choose a word set...</option>' + wordSetOptions;
  }
}
...
function renderAssignmentsTable() {
  ...
  if (assignments.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="6" style="text-align: center; color: #64748b;">No assignments created yet.</td></tr>';
    return;
  }
  ...
  tbody.innerHTML = assignments
    .map((assignment) => {
      ...
      const hasCompleted = results.some((result) => {
        ...
        const assignmentTime = assignment.assignedAt
...
function renderAnalytics() {
  ...
  if (results.length === 0) {
    ...
    document
      .querySelector("#analytics .section-card .section-title")
      .parentNode.querySelector("div")
      .appendChild(testDataBtn);
  }
}
...
function setupEventListeners() {
  // Word Sets
  document
    .getElementById("createWordSetBtn")
    ?.addEventListener("click", showCreateWordSetModal);
  document
    .getElementById("cleanupWordSetsBtn")
    ?.addEventListener("click", cleanupWordSets);

  // Students & Classes
  document
    .getElementById("createClassBtn")
    ?.addEventListener("click", showCreateClassModal);
  document
  ...
  // Assignments
  document
    .getElementById("assignToStudentBtn")
    ?.addEventListener("click", assignToStudent);
  document
    .getElementById("assignToClassBtn")
    ?.addEventListener("click", assignToClass);
  document
    .getElementById("refreshAssignmentsBtn")
    ?.addEventListener("click", refreshAssignmentStatus);

  // Analytics
  document
    .getElementById("deleteAllResultsBtn")
    ?.addEventListener("click", deleteAllResults);
  ...
  document
    ...
    .addEventListener("click", exportScreenshot);
...
async function createWordSet() {
  ...
  const description = document
    .getElementById("wordSetDescription")
    .value.trim();
...
async function updateWordSet(setId) {
  ...
  const description = document
    .getElementById("editWordSetDescription")
    .value.trim();
  ...
  try {
    ...
    const spellingDoc = await window.db
      .collection("spelling")
      .doc("wordlist")
      .get();
...
async function updateClass(classId) {
  ...
  const description = document
    .getElementById("editClassDescription")
    .value.trim();
...
async function assignToStudent() {
  ...
  const existingAssignment = assignments.find(
    (a) => a.studentId === studentId && a.wordSetId === wordSetId,
  );
  if (existingAssignment) {
    showNotification(
      "This student already has this word set assigned",
      "warning",
    );
    return;
  }
  ...
  try {
    const firebaseCheck = await window.db
    ...
    if (!firebaseCheck.empty) {
      showNotification(
        "This assignment already exists in the database",
        "warning",
      );
  ...
  try {
    ...
    showNotification(
      `Successfully assigned "${wordSetName}" to ${studentName}!`,
      "success",
    );
...
async function assignToClass() {
  ...
  const existingClassAssignment = assignments.find(
    (a) =>
      a.classId === classId && a.wordSetId === wordSetId && a.type === "class",
  );
  if (existingClassAssignment) {
    showNotification(
      `Class "${className}" already has "${wordSetName}" assigned`,
      "warning",
    );
    return;
  }
  ...
  try {
    ...
    if (!firebaseCheck.empty) {
      showNotification(
        "This class assignment already exists in the database",
        "warning",
      );
      await loadAssignments();
      renderAssignments();
  ...
  try {
    ...
    showNotification(
      `✅ Successfully assigned "${wordSetName}" to class "${className}" (${classStudents.length} students)!`,
      "success",
    );
...
async function deleteStudent(studentId) {
  if (
    !confirm(
      "Are you sure you want to delete this student? All their data will be removed.",
    )
  ) {
    return;
  }
...
async function deleteAllResults() {
  ...
  if (
    !confirm(
      "⚠️ WARNING: This will permanently delete ALL quiz results from ALL students!\n\nThis action cannot be undone. Are you absolutely sure you want to continue?",
    )
  ) {
    return;
  }
  ...
  const password = prompt(
    "🔐 To confirm this dangerous action, please enter the teacher password:",
  );
  ...
  if (password !== "9739") {
    if (password !== null) {
      // User didn't cancel
      showNotification(
        "❌ Incorrect password! Delete operation cancelled for security.",
        "error",
      );
    }
    return;
  ...
  if (confirmText !== "DELETE ALL DATA") {
    if (confirmText !== null) {
      // User didn't cancel
      showNotification(
        "❌ Confirmation text does not match. Delete operation cancelled.",
        "error",
      );
    }
    return;
  ...
  try {
    ...
    showNotification(
      "✅ All quiz results have been permanently deleted!",
      "success",
    );

    // Log this action for security purposes
    console.log(
      `[SECURITY] All quiz results deleted at ${new Date().toISOString()}`,
    );
  } catch (error) {
    console.error("Error deleting all results:", error);
    showNotification(
      "❌ Error deleting all results. Please try again.",
      "error",
    );
  }
}
...
function quickAssignWordSet(wordSetId) {
  ...
  setTimeout(() => {
    ...
    showNotification(
      "Word set selected for assignment. Choose a student or class above.",
      "info",
    );
  }, 100);
}
...
async function cleanupWordSets() {
  try {
    ...
    const problematicSets = wordSets.filter((set) => {
      ...
      const isTestSet =
        words.length <= 3 && words.every((word) => word.length <= 1);
      return isTestSet;
    });
    ...
    if (problematicSets.length > 0) {
      ...
      for (const set of problematicSets) {
        console.log(
          `Deleting problematic word set: ${set.name} with words:`,
          set.words,
        );
      ...
      await loadAllData();
      showNotification(
        `Cleaned up ${problematicSets.length} problematic word sets`,
        "success",
      );
    } else {
...
function initializeAnalyticsFilters() {
  ...
  document.getElementById("analyticsFromDate").value = thirtyDaysAgo
    .toISOString()
    .split("T")[0];
  document.getElementById("analyticsToDate").value = today
    .toISOString()
    .split("T")[0];
  ...
  // Setup event listeners
  document
    .getElementById("analyticsFilterType")
    .addEventListener("change", updateAnalyticsFilterOptions);
  document
    .getElementById("applyAnalyticsFilter")
    .addEventListener("click", applyAnalyticsFilter);
  document
    .getElementById("analyticsSortBy")
...
function updateAnalyticsFilterOptions() {
  ...
  if (filterType === "student") {
    ...
    const studentNamesFromResults = [
      ...new Set(results.map((result) => result.user)),
    ...
    if (allStudentNames.length > 0) {
      ...
      filterValue.innerHTML +=
        '<option value="" disabled>No students found - students need to complete quizzes first</option>';
    }
    ...
    if (classes.length > 0) {
      ...
      filterValue.innerHTML +=
        '<option value="" disabled>No classes found - create classes first</option>';
    }
  }
...
function showStudentSelectionModal() {
  ...
  if (allStudentNames.length === 0) {
    showNotification(
      "No students found. Students need to complete quizzes first.",
      "warning",
    );
    return;
  }
        ...
        <div style="max-height: 400px; overflow-y: auto; border: 1px solid #e0e7ef; border-radius: 8px; padding: 8px;">
            ${allStudentNames
              .map((studentName) => {
                ...
                const studentQuizzes = results.filter(
                  (r) =>
                    r.user &&
                    r.user.toLowerCase().trim() ===
                ...
                const studentClass = studentRecord
                  ? classes.find((c) => c.id === studentRecord.classId)
                  : null;
...
function showClassSelectionModal() {
  if (classes.length === 0) {
    showNotification(
      "No classes found. Create classes first in the Students & Classes tab.",
      "warning",
    );
    return;
  }
            ...
            ${classes
              .map((cls) => {
                ...
                const classQuizzes = results.filter((r) => {
                  const studentNames = studentsInClass.map((s) =>
                    s.name.toLowerCase().trim(),
                  );
                  return (
                    r.user && studentNames.includes(r.user.toLowerCase().trim())
                  );
                });
                          ...
                          studentsInClass.length > 0
                            ...
                            <div style="margin-top: 8px; font-size: 0.8rem; color: #64748b;">
                                Students: ${studentsInClass
                                  ...
                                  .join(
                                    ", ",
                                  )}${studentsInClass.length > 3 ? ` +${studentsInClass.length - 3} more` : ""}
                            </div>
                        `
                            : ""
                        }
...
function selectStudent(studentName) {
  ...
  const existingOption = Array.from(filterValue.options).find(
    (option) => option.value === studentName,
  );
  if (!existingOption) {
...
function selectClass(classId) {
  ...
  showNotification(
    `Filtered analytics for class: ${selectedClass.name}`,
    "success",
  );
}
...
function applyAnalyticsFilter() {
  ...
  if (filterType === "student" && filterValue) {
    ...
    filteredResults = filteredResults.filter(
      (result) =>
        result.user &&
        result.user.toLowerCase().trim() === filterValue.toLowerCase().trim(),
    );
  ...
  filteredResults.sort((a, b) => {
    switch (sortBy) {
      ...
      case "wordset_asc":
        const aWordSet =
          wordSets.find((ws) => ws.id === a.wordSetId)?.name || "Unknown";
        const bWordSet =
          wordSets.find((ws) => ws.id === b.wordSetId)?.name || "Unknown";
      ...
      case "totaltime_asc":
        const aTotalTime =
          ...
          (a.startTime && a.finishTime
            ? Math.round(
                (new Date(a.finishTime) - new Date(a.startTime)) / 1000,
              )
            : 0);
        const bTotalTime =
          b.totalTimeSeconds ||
          (b.startTime && b.finishTime
            ? Math.round(
                (new Date(b.finishTime) - new Date(b.startTime)) / 1000,
              )
            : 0);
        return aTotalTime - bTotalTime;
      case "totaltime_desc":
        const aTotalTimeDesc =
          a.totalTimeSeconds ||
          (a.startTime && a.finishTime
            ? Math.round(
                (new Date(a.finishTime) - new Date(a.startTime)) / 1000,
  ...
  showNotification(
    "Analytics filter and sorting applied successfully!",
    "success",
  );
}
...
function updateFilteredAnalytics() {
  ...
  if (filterType === "student") {
    ...
    const uniqueStudents = [...new Set(filteredResults.map((r) => r.user))]
      .length;
    const studentsElement = document.getElementById("filteredStudents");
  ...
  const uniqueWordSets = [
    ...new Set(filteredResults.map((r) => r.wordSetId).filter((id) => id)),
  ].length;
...
function generateInsights() {
  ...
  if (filteredResults.length === 0) {
    insightsDiv.innerHTML =
      '<p style="color: #64748b; margin: 0;">No data available for the selected filters.</p>';
  ...
  if ((filterType === "student" || filterType === "class") && filterValue) {
    const insights = generateIndividualSessionInsights(
      filteredResults,
      filterType,
      filterValue,
    );
...
function generateIndividualSessionInsights(results, filterType, filterValue) {
  let insights = "";
  const studentName =
    filterType === "student" && filterValue ? filterValue : null; // filterValue is now the student name directly
  const className =
    filterType === "class" && filterValue
      ? classes.find((c) => c.id === filterValue)?.name
      : null; // filterValue is the class ID, convert to name
...
function analyzeIndividualSession(result, sessionNumber) {
  ...
  const score =
    totalWords > 0 ? Math.round((firstTryCorrect / totalWords) * 100) : 0;
  const hintsUsed = words.filter((w) => w.hint).length;
  const hintPercentage =
    totalWords > 0 ? Math.round((hintsUsed / totalWords) * 100) : 0;
...
function generateProgressSummary(sortedResults, filterType) {
  ...
  const recentAvg =
    recentSessions.reduce((sum, result) => {
      const words = result.words || [];
      const firstTryCorrect = words.filter((w) => {
        const attempts = w.attempts || [];
        return attempts.length > 0 && attempts[0] === w.word;
      ...
      const score =
        words.length > 0 ? (firstTryCorrect / words.length) * 100 : 0;
      return sum + score;
    }, 0) / recentSessions.length;

  const olderAvg =
    olderSessions.reduce((sum, result) => {
      const words = result.words || [];
      const firstTryCorrect = words.filter((w) => {
        const attempts = w.attempts || [];
        return attempts.length > 0 && attempts[0] === w.word;
  ...
  const trendColor =
    improvement > 5 ? "#22c55e" : improvement < -5 ? "#ef4444" : "#3b82f6";
...
function renderFilteredAnalyticsTable() {
  ...
  if (validResults.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="6" style="text-align: center; color: #64748b;">No valid quiz results found. Only assigned word sets are displayed.</td></tr>';
    return;
  }
  ...
  const sortBy = document.getElementById("analyticsSortBy").value;
  const sortLabel =
    document.getElementById("analyticsSortBy").selectedOptions[0].text;
  ...
  tbody.innerHTML = displayResults
    .map((result) => {
      const words = result.words || [];
      ...
      if (result.date) {
        ...
        if (result.startTime && result.finishTime) {
          ...
          startTimeRow = startTime.toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          });
          finishTimeRow = finishTime.toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          });
        } else {
          // Fallback to single time
          startTimeRow = date.toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          });
          finishTimeRow = "";
      ...
      if (mistakesAndHints.length > 0) {
        detailsHtml +=
          '<div style="font-weight:600;color:#dc2626;margin-bottom:6px;">📝 Learning Areas (' +
          mistakesAndHints.length +
          "):</div>";
        ...
        mistakesAndHints.forEach((w) => {
          ...
          if (!w.correct) {
            ...
            const wrongAttempt =
              w.attempts && w.attempts.length > 0
                ? w.attempts[0]
                : "no attempt";
            detailsHtml +=
              '<span style="background:#fee2e2;color:#dc2626;padding:1px 3px;border-radius:3px;text-decoration:line-through;font-weight:600;">' +
              wrongAttempt +
              "</span>";
            detailsHtml += ' <span style="color:#6b7280;">→</span> ';
            detailsHtml +=
              '<span style="background:#dcfce7;color:#059669;padding:1px 3px;border-radius:3px;font-weight:600;">' +
              w.word +
              "</span>";
          } else if (w.hint) {
            ...
            detailsHtml +=
              wordDisplay +
              ' <span style="color:#d97706;font-size:0.75rem;">(used hint)</span>';
          }
        ...
        detailsHtml +=
          '<div style="color:#059669;font-weight:600;">🎉 Perfect! All words spelled correctly without hints</div>';
      }
            ...
            <tr>
                ...
                <td class="word-set-cell multi-row-cell">
                    ...
                    ${(() => {
                      ...
                      const wordSet = wordSets.find(
                        (ws) => ws.id === result.wordSetId,
                      );
                      if (wordSet && wordSet.words) {
                        const wordList = wordSet.words.slice(0, 12).join(", "); // Show first 12 words for dashboard
                        const moreWords =
                          wordSet.words.length > 12
                            ? ` (+${wordSet.words.length - 12} more)`
                            : "";
                        return `
...
function analyzePerformanceData(results) {
  ...
  if (results.length >= 2) {
    ...
    const firstHalf = sortedResults.slice(
      0,
      Math.ceil(sortedResults.length / 2),
    );
    ...
    const firstAvg =
      firstHalf.reduce((sum, r) => {
        ...
        return (
          sum + (words.length > 0 ? (firstTryCorrect / words.length) * 100 : 0)
        );
      }, 0) / firstHalf.length;
...
function generateInsightsFromAnalysis(analysis, filterType, filterValue) {
  let insights = "";
  const studentName =
    filterType === "student" && filterValue
      ? students.find((s) => s.id === filterValue)?.name
      : null;
  const className =
    filterType === "class" && filterValue
      ? classes.find((c) => c.id === filterValue)?.name
  ...
  if (analysis.progressTrend && analysis.progressTrend.change !== undefined) {
    const trendIcon = analysis.progressTrend.improving
      ...
      : analysis.progressTrend.stable
        ? "➡️"
        : "📉";
    const trendColor = analysis.progressTrend.improving
      ? "#22c55e"
      : analysis.progressTrend.stable
        ? "#3b82f6"
        : "#ef4444";
    const trendText = analysis.progressTrend.improving
      ? "improving"
      : analysis.progressTrend.stable
        ? "stable"
        : "declining";
...
async function setClassDefaultWordSet(classId) {
  try {
    ...
    const wordSetName = wordSetId
      ? wordSets.find((ws) => ws.id === wordSetId)?.name
      : "None";
    const className = classes.find((c) => c.id === classId)?.name;
    ...
    if (!confirm(confirmMessage)) {
      ...
      const currentDefault = classes.find(
        (c) => c.id === classId,
      )?.defaultWordSetId;
    ...
    if (studentsInClass.length > 0) {
      console.log(
        `Updating default word set for ${studentsInClass.length} students in class`,
      );
    ...
    if (updatedStudentsCount > 0) {
      showNotification(
        `Default word set for class "${className}" and ${updatedStudentsCount} students updated to: ${wordSetName}`,
        "success",
      );
    } else {
      showNotification(
        `Default word set for class "${className}" updated to: ${wordSetName}`,
        "success",
      );
    }
...
async function setStudentDefaultWordSet(studentId) {
  try {
    ...
    const studentName = students.find((s) => s.id === studentId)?.name;
    showNotification(
      `Default word set for ${studentName} updated to: ${wordSetName}`,
      "success",
    );
...
async function syncClassAssignments(classId) {
  try {
    ...
    if (
      studentsWithoutAssignments.length === 0 &&
      studentsWithoutDefaults.length === 0
    ) {
    ...
    if (classData.defaultWordSetId) {
      const classDefaultWordSet = wordSets.find(
        (ws) => ws.id === classData.defaultWordSetId,
      );
      ...
      if (choice === "1" || choice === "3") {
        ...
        for (const student of studentsWithoutAssignments) {
          try {
            ...
            console.error(
              `Error creating assignment for ${student.name}:`,
              error,
            );
          }
        }
        showNotification(
          `Created ${assignmentsCreated} new assignments`,
          "success",
        );
      }
      ...
      if (choice === "2" || choice === "3") {
        ...
        for (const student of studentsWithoutDefaults) {
          try {
            ...
            const studentIndex = students.findIndex((s) => s.id === student.id);
            if (studentIndex !== -1) {
              students[studentIndex].defaultWordSetId =
                classData.defaultWordSetId;
            }
            defaultsSet++;
          } catch (error) {
  ...
  try {
    ...
    for (const cls of classes) {
      ...
      for (const student of studentsInClass) {
        ...
        if (!hasAssignment) {
          ...
          issueDetails.push(
            `${student.name} (${cls.name}) has no individual assignment`,
          );
        }
        ...
        if (!student.defaultWordSetId) {
          ...
          issueDetails.push(
            `${student.name} (${cls.name}) has no default word set`,
          );
        }
      }
    }
    ...
    for (const student of studentsWithoutClass) {
      ...
      if (!hasAssignment) {
        studentsWithoutAssignments++;
        issueDetails.push(
          `${student.name} (no class) has no individual assignment`,
        );
      }
    ...
    if (issueDetails.length === 0) {
      ...
      showModal(
        ...
        `<pre style="white-space: pre-wrap; font-family: monospace; font-size: 0.9rem;">${report}</pre>`,
      );
    }
  } catch (error) {
...
function exportAnalyticsData() {
  ...
  validResults.forEach((result, index) => {
    ...
    doc.setFont(undefined, "bold");
    doc.text(
      `${index + 1}. ${result.user || "Unknown"} - ${result.wordSetName || "Unknown Set"}`,
      20,
      yPos,
    );
    yPos += 8;
    ...
    yPos += 6;
    doc.text(
      `Score: ${fraction} (${score}%) | Time: ${result.totalTimeSeconds || 0}s`,
      20,
      yPos,
    );
    yPos += 12;
    ...
    if (mistakesAndHints.length > 0) {
      ...
      mistakesAndHints.forEach((w) => {
        ...
        if (!w.correct) {
          ...
          const wrongAttempt =
            w.attempts && w.attempts.length > 0 ? w.attempts[0] : "no attempt";
          practiceText = `• MISTAKE: "${wrongAttempt}" → "${w.word}" (incorrect spelling)`;
        } else if (w.hint) {
          // Show word with hint information
          if (w.hintLetters && w.hintLetters.length > 0) {
            const hintedLetters = w.hintLetters
              .map((pos) => `${w.word[pos]} (position ${pos + 1})`)
              .join(", ");
      ...
      doc.text(
        "🎉 PERFECT! All words spelled correctly without hints",
        25,
        yPos,
      );
...
async function exportScreenshot() {
  try {
    ...
    if (filterType === "student" && filterValue) {
      ...
      const className =
        classes.find((c) => c.id === filterValue)?.name || "Unknown Class";
    ...
    if (exportOptions.mode === "separate") {
      ...
      const uniqueStudents = [...new Set(validResults.map((r) => r.user))]
        .filter((name) => name)
        .sort();
      ...
      showNotification(
        `Creating individual reports for ${uniqueStudents.length} students...`,
        "info",
      );

      for (
        let studentIndex = 0;
        studentIndex < uniqueStudents.length;
        studentIndex++
      ) {
        const studentName = uniqueStudents[studentIndex];
        const studentResults = validResults.filter(
          (r) => r.user === studentName,
        );
        ...
        const tableRows = studentResults
          .map((result, index) => {
            const words = result.words || [];
            const firstTryCorrect = words.filter((w) => {
              const attempts = w.attempts || [];
            ...
            if (mistakesAndHints.length > 0) {
              learningDetails = mistakesAndHints
                .map((w) => {
                  let wordDetail = "";
              ...
              learningDetails =
                '<span style="color: #059669; font-weight: 600; font-size: 18px;">🎉 Perfect! All words correct without hints</span>';
            }
        ...
        try {
          ...
          console.error(
            `Error generating screenshot for ${studentName}:`,
            error,
          );
          showNotification(
            `Error generating report for ${studentName}`,
            "error",
          );
        } finally {
      ...
      if (screenshots.length > 0) {
        showNotification(
          `Successfully created ${screenshots.length} individual student reports!`,
          "success",
        );
      }
    } else {
...
function showItemsPerPageModal() {
  return new Promise((resolve) => {
    ...
    // Handle confirm
    document
      .getElementById("confirmItemsPerPage")
    ...
    // Handle cancel
    document
      .getElementById("cancelItemsPerPage")
      .addEventListener("click", () => {
        closeModal();
        resolve(null);
      });
...
async function executeQuickAssignToClass(classId) {
  ...
  if (existingClassAssignment) {
    showNotification(
      `Class "${classData.name}" already has this word set assigned`,
      "warning",
    );
    closeModal();
    return;
  ...
  try {
    ...
    const wordSetName = wordSets.find((ws) => ws.id === wordSetId)?.name;
    showNotification(
      `✅ Successfully assigned "${wordSetName}" to class "${classData.name}" (${studentsInClass.length} students)!`,
      "success",
    );
  } catch (error) {
    console.error("Error creating class assignment:", error);
...
async function deleteClassAssignment(assignmentId) {
  if (
    !confirm(
      "Are you sure you want to remove this class assignment? This will also remove it from all students who received it from this class.",
    )
  ) {
    return;
  }
  ...
  try {
    ...
    const relatedStudentAssignments = assignments.filter(
      (a) =>
    ...
    renderAssignments();
    showNotification(
      "Class assignment and related student assignments removed successfully!",
      "success",
    );
  } catch (error) {
...
function toggleStudentAssignments(studentId) {
  ...
  if (toggleBtn.textContent.trim() === "Show All") {
    ...
    container.innerHTML = renderAssignmentsList(
      studentAssignments,
      "individual",
    );
...
function addTestData() {
  ...
  showNotification(
    "Test data feature is for demonstration only. Real data comes from student quiz completions.",
    "info",
  );
}
...
async function cleanupDuplicateAssignments() {
  if (
    !confirm(
      "This will scan and remove duplicate assignments from the database.\n\nDuplicates are identified by:\n- Same student + same word set\n- Same class + same word set\n\nThis action cannot be undone. Continue?",
    )
  ) {
    return;
  }
  ...
  try {
    ...
    if (
      !confirm(
        `Found ${duplicatesToDelete.length} duplicate assignments.\n\nDo you want to delete them now?`,
      )
    ) {
      return;
    }
    ...
    for (let i = 0; i < duplicatesToDelete.length; i += batchSize) {
      ...
      showNotification(
        `Deleted ${deletedCount}/${duplicatesToDelete.length} duplicates...`,
        "info",
      );
    }

    showNotification(
      `Successfully removed ${duplicatesToDelete.length} duplicate assignments!`,
      "success",
    );
    ...
    showNotification(
      "Error cleaning up duplicates. Please try again.",
      "error",
    );
  }
}
...
async function bulkDeleteClasses() {
  ...
  try {
    ...
    showNotification(
      `Successfully deleted ${selectedItems.classes.size} classes and all related data`,
      "success",
    );
    selectedItems.classes.clear();
...
async function bulkDeleteStudents() {
  ...
  if (
    !confirm(
      `Are you sure you want to delete ${selectedItems.students.size} students?\n\nStudents to delete:\n${studentNames.join("\n")}\n\nThis will also delete all their individual assignments.`,
    )
  ) {
    return;
  }
  ...
  try {
    ...
    showNotification(
      `Successfully deleted ${selectedItems.students.size} students and their assignments`,
      "success",
    );
    selectedItems.students.clear();
    bulkSelectionMode = false;
...
async function deleteAllClassAssignments(classId) {
  ...
  if (
    !confirm(
      `Are you sure you want to delete ALL ${classAssignments.length} assignments for class "${cls.name}"?\n\nThis action cannot be undone.`,
    )
  ) {
    return;
  }
  ...
  try {
    ...
    showNotification(
      `Successfully deleted all ${classAssignments.length} assignments for class "${cls.name}"`,
      "success",
    );
    await loadAllData();
    renderStudentsAndClasses();
    ...
    showNotification(
      "Error deleting class assignments. Please try again.",
      "error",
    );
  }
}
...
async function deleteAllStudentAssignments(studentId) {
  ...
  if (studentAssignments.length === 0) {
    showNotification(
      "No individual assignments to delete for this student",
      "info",
    );
    return;
  }
  ...
  if (
    !confirm(
      `Are you sure you want to delete ALL ${studentAssignments.length} individual assignments for "${student.name}"?\n\nThis will not affect class assignments.\nThis action cannot be undone.`,
    )
  ) {
    return;
  }
  ...
  try {
    ...
    showNotification(
      `Successfully deleted all ${studentAssignments.length} individual assignments for "${student.name}"`,
      "success",
    );
    await loadAllData();
    renderStudentsAndClasses();
    ...
    showNotification(
      "Error deleting student assignments. Please try again.",
      "error",
    );
  }
}
...
async function createAssignmentWithValidation(assignmentData, retryCount = 0) {
  ...
  try {
    ...
    if (!isConnected) {
      throw new Error(
        "No connection to database. Please check your internet connection.",
      );
    }
    ...
    if (
      ...
      console.log(
        `Assignment creation failed, retrying... (${retryCount + 1}/${maxRetries})`,
      );
...
async function executeBulkMove() {
  const selectedWordSets = Array.from(
    document.querySelectorAll(".bulk-move-checkbox:checked"),
  ).map((cb) => cb.value);
  ...
  try {
    ...
    const targetFolderName = targetFolderId
      ? wordSetFolders.find((f) => f.id === targetFolderId)?.name ||
        "Unknown Folder"
      : "Root Level";

    showNotification(
      `Successfully moved ${selectedWordSets.length} word set(s) to ${targetFolderName}!`,
      "success",
    );
  } catch (error) {
